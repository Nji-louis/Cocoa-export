(function (global) {
  ...
  function renderRichHtml(value) {
    ...
    const walk = global.document.createTreeWalker(wrapper, NodeFilter.SHOW_ELEMENT, null);
    const toRemove = [];
    while (walk.nextNode()) {
      ...
      const permitted = allowedAttrs[node.tagName] || new Set();
    ...
    return wrapper.innerHTML.trim() || '<p>No content available yet.</p>';
  }
  ...
  async function renderBlogListPage() {
    ...
    try {
      ...
      const cards = await Promise.all(posts.map(async function (post) {
        ...
        const sourceTitle = escapeHtml(post.source_title || "Original news source");
        const dateLabel = formatDate(post.published_at);
        return [
          '<div class="col-sm-6">',
          ...
          '     <small style="display:block;margin-bottom:6px;color:#666;">Source: ' + sourceTitle + "</small>",
  ...
  async function renderBlogDetailPage() {
    ...
    try {
      ...
      if (contentNode) {
        contentNode.innerHTML = renderRichHtml(post.content || post.excerpt || "");
      }
      const sourceNode = document.getElementById("blog-detail-source-copy");
      if (sourceNode) {
        const sourceUrl = String(post.source_url || "").trim();
        const sourceTitle = escapeHtml(post.source_title || "Original news source");
        sourceNode.innerHTML = sourceUrl
          ? 'This article is built from the latest news coverage. <strong>Source:</strong> ' + sourceTitle + ' - <a href="' + sanitizeAttribute(sourceUrl) + '" target="_blank" rel="noopener noreferrer">Open original news coverage</a>'
          : "This article is built from the latest news coverage, but an original link was not available.";
      }
      const tagsNode = document.getElementById("blog-detail-tags");
      if (tagsNode) {
        const tags = Array.isArray(post.suggested_tags) ? post.suggested_tags.filter(Boolean) : [];
        tagsNode.textContent = tags.length ? "Tags: " + tags.join(", ") : "Tags will appear here once the article loads.";
      }
      const relatedNode = document.getElementById("blog-detail-related");
      if (relatedNode) {
        const related = Array.isArray(post.related_articles) ? post.related_articles.filter(Boolean) : [];
        if (related.length > 0) {
          relatedNode.innerHTML = [
            '<div class="blog-detail-related-list">',
            related.slice(0, 3).map(function (item) {
              const slug = item && item.slug ? String(item.slug).trim() : "";
              const titleText = escapeHtml(item && item.title ? item.title : "Related article");
              const excerptText = escapeHtml(item && item.excerpt ? item.excerpt : "");
              const link = slug ? getPostUrl(slug) : "blog.html";
              return [
                '<p><a href="' + link + '">' + titleText + '</a></p>',
                excerptText ? '<p style="margin-top:-8px;color:#666;">' + excerptText + '</p>' : "",
              ].join("");
            }).join(""),
            "</div>",
          ].join("");
        } else {
          relatedNode.innerHTML = '<p>Related articles will appear here once the article loads.</p>';
        }
      }
