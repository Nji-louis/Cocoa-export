(function (global) {
  ...
  ns.adminApi = {
    async upsertListing(payload) {
      ...
      const { data, error } = await client.functions.invoke("admin-upsert-listing", {
        body: payload,
      });
    ...
    async createSignedUrl(payload) {
      const client = await requireClient();
      const { data, error } = await client.functions.invoke("generate-document-signed-url", {
        body: payload,
      });
      if (error) throw error;
      if (data && data.error) throw new Error(data.error);
    ...
    async listProducts(params) {
      ...
      const pageSize = Math.min(100, Math.max(1, Number((params && params.pageSize) || 50)));
    ...
    async uploadProductImage(options) {
      ...
      const bucket = ((options && options.bucket) || "product-images").trim();
      const productSlug = String((options && options.productSlug) || "product")
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "-")
        .replace(/^-+|-+$/g, "") || "product";
