(function (global) {
  ...
  ns.authApi = {
    ...
    async getMyRoles(user) {
      ...
      const { data: legacyProfile, error: legacyProfileError } = await client
        ...
        .eq("id", resolvedUser.id)
        .maybeSingle();
      if (!legacyProfileError && legacyProfile && legacyProfile.role) {
        roles.add(String(legacyProfile.role).toLowerCase());
      }
      if (roles.size === 0) {
        roles.add("buyer");
      }
  ...
  function normalizeError(error, fallbackMessage) {
    ...
    if (typeof error === "string") {
      if (/failed to fetch|networkerror|load failed|fetch failed/i.test(error)) {
  ...
  function getSafeRedirectTo() {
    ...
    return ns.resolveEmailRedirectUrl("/auth/login.html") || undefined;
  }
  ...
  function isAuthEntryPage() {
    ...
    return /\/auth\/login\.html$/i.test(global.location.pathname);
  }
  ...
  async function handleAuthRedirect(eventName, user, roles, flowType) {
    if (!shouldRedirectAfterAuth(eventName, user, flowType)) {
      return;
    }
    if (ns.authController && typeof ns.authController.beforePostAuthRedirect === "function") {
      const shouldContinue = await ns.authController.beforePostAuthRedirect({
        event: eventName,
        flowType: flowType,
        user: user,
        roles: roles,
  ...
  function ensureNavControls() {
    ...
    if (!document.getElementById(AUTH_NAV_PORTAL_ID)) {
      ...
      portalLi.innerHTML = '<a class="tag_m" href="/admin/dashboard.html">Portal</a>';
    ...
    if (!document.getElementById(AUTH_NAV_STAFF_ID)) {
      ...
      staffLi.innerHTML = '<a class="tag_m" href="/admin/dashboard.html#users">Staff</a>';
    ...
    if (!document.getElementById(AUTH_NAV_ADMIN_ID)) {
      ...
      adminLi.innerHTML = '<a class="tag_m" href="/admin/dashboard.html#settings">Admin</a>';
  ...
  async function applyAuthState(eventName, session, options) {
    ...
    await handleAuthRedirect(eventName, user, roles, flowType);
  }
