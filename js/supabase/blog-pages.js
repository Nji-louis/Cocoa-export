(function (global) {
  ...
  function extractKeyword(post) {
    const candidates = [
      post && post.seo_keyword,
      post && post.primary_keyword,
    ...
    for (let index = 0; index < candidates.length; index += 1) {
      const normalized = String(candidates[index] || "").trim();
      if (!normalized) continue;
      return normalized.replace(/-/g, " ");
    }

    return "Cameroon cocoa export";
  }

  function extractReadingTime(post) {
    const numericCandidates = [
      post && post.reading_time_minutes,
      post && post.read_time_minutes,
      post && post.reading_time,
    ];

    for (let index = 0; index < numericCandidates.length; index += 1) {
      const value = Number(numericCandidates[index]);
      if (Number.isFinite(value) && value > 0) {
        return Math.max(1, Math.round(value)) + " min read";
      }
    }

    const content = String((post && (post.content || post.excerpt || "")) || "").trim();
    if (!content) {
      return "4 min read";
    }
  ...
  function dispatchBlogHubUpdated() {
    if (!global.document || typeof global.document.dispatchEvent !== "function") {
      return;
    }

    try {
      global.document.dispatchEvent(new CustomEvent("app-blog-hub-updated"));
    } catch (error) {
      const event = global.document.createEvent("Event");
      event.initEvent("app-blog-hub-updated", true, true);
      global.document.dispatchEvent(event);
    }
  }

  async function renderHubBlogCards(target, posts) {
    if (!target) return;

    if (!Array.isArray(posts) || posts.length === 0) {
      target.innerHTML = '<p class="blog-hub-empty-message">No blog posts published yet.</p>';
      dispatchBlogHubUpdated();
      return;
    }
    ...
    const cards = await Promise.all(posts.map(async function (post) {
      ...
      const readingTime = extractReadingTime(post);

      return [
        '<article class="blog-hub-card" data-category="' + sanitizeAttribute(category) + '" data-keywords="' + sanitizeAttribute(keyword) + '" data-title="' + sanitizeAttribute(post.title || "") + '" data-excerpt="' + sanitizeAttribute(post.excerpt || "") + '">',
        '  <a class="blog-hub-card-image" href="' + link + '"><img loading="lazy" src="' + cover + '" alt="' + title + '"></a>',
        '  <div class="blog-hub-card-body">',
        '    <div class="blog-card-meta"><span class="blog-card-category">' + escapeHtml(category) + '</span><span class="blog-card-date">' + dateLabel + '</span></div>',
        '    <p class="blog-card-keyword">' + escapeHtml(keyword) + '</p>',
        '    <h2><a href="' + link + '">' + title + '</a></h2>',
        '    <p>' + excerpt + '</p>',
        '    <div class="blog-card-footer"><span class="blog-card-reading">' + escapeHtml(readingTime) + '</span><a class="button_1 blog-card-button" href="' + link + '"><span>READ MORE</span> <i class="fa fa-chevron-right"></i></a></div>',
        '  </div>',
        '</article>',
      ].join("");
    }));
  ...
  async function renderBlogListPage() {
    const hubGrid = document.getElementById("blog-hub-grid");
    const target = hubGrid || document.getElementById("blog-post-list");
    if (!target) return;
    ...
    if (hubGrid) {
      target.innerHTML = '<p class="blog-hub-empty-message">Loading blog posts...</p>';
    } else {
      target.innerHTML = '<p style="padding:15px;">Loading blog posts...</p>';
    }

    try {
      const response = await ns.blogApi.listPublishedPosts({ page: 1, pageSize: 50 });
      const posts = response.items || [];

      if (hubGrid) {
        await renderHubBlogCards(target, posts);
        return;
      }
      ...
      if (hubGrid) {
        target.innerHTML = '<p class="blog-hub-empty-message" style="color:#b10000;">' + escapeHtml(message) + '</p>';
      } else {
  ...
  async function renderBlogDetailPage() {
    ...
    if (detailRoot.dataset && detailRoot.dataset.requiresSlug === "true" && !getSlugFromUrl()) {
      return;
    }
