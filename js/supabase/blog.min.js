(function (global) {
  ...
  async function invokeBlogCommentWithFetch(client, payload) {
    const config = getConfig();
    if (!config || !config.url || !config.anonKey) {
      throw new Error("Supabase client is not configured");
    }
    ...
    let response;
    try {
      response = await fetch(config.url + "/functions/v1/submit-blog-comment", {
        method: "POST",
        headers,
  ...
  ns.blogApi = {
    ...
    async submitComment(payload) {
      const client = await requireClient();
      try {
        return await invokeBlogCommentWithFetch(client, payload);
      } catch (fetchError) {
        const { data, error } = await client.functions.invoke("submit-blog-comment", {
          body: payload,
        });
        if (error) throw fetchError || error;
